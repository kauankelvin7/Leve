  ...
  return (
    <main className="today-page">
      ...
      <section className="day-overview" aria-label="Resumo do dia selecionado">
        ...
        <div className="day-overview-content"><p className="eyebrow">Resumo</p><h2>{loading ? 'Abrindo o dia…' : pendingTaskCount ? `${pendingTaskCount} ${pendingTaskCount === 1 ? 'tarefa' : 'tarefas'} ${selectedDay === today ? 'para hoje' : 'neste dia'}` : taskCount ? 'Checklist em dia' : pendingCount ? `${pendingCount} ${pendingCount === 1 ? 'compromisso' : 'compromissos'} neste dia` : 'Nada planejado'}</h2><p>{taskCount ? `${completedTaskCount} de ${taskCount} concluídas.` : plannedMinutes ? `${plannedMinutes} min planejados.` : 'Dia livre por enquanto.'}</p><nav className="day-shortcuts" aria-label="Acessos rápidos"><Link to="/notas"><Icon name="note" />Notas</Link><Link to="/compras"><Icon name="basket" />Compras</Link><Link to="/revisao"><Icon name="clock" />Tempo registrado</Link></nav></div>
      </section>
