...
export function useCalendarRange({ startDate, endDate, categoryId = '', pageSize = 50 }: CalendarRangeOptions) {
  ...
  const live = useLiveQueries(key, () => {
    ...
    return [
      query(root, where('schedule.dueDate', '>=', startDate), where('schedule.dueDate', '<=', endDate), limit(pageSize)),
      query(root, where('schedule.startDate', '<=', endDate), where('schedule.endDate', '>=', startDate), limit(pageSize)),
      query(root, where('schedule.startDate', '<=', endDate), where('schedule.endDateExclusive', '>', startDate), limit(pageSize)),
    ];
  }, pageSize);
