...
export function Trash() {
  ...
  return <main><header className="page-heading"><p className="eyebrow">Itens guardados por 30 dias</p><h1 id="page-title" tabIndex={-1}>Lixeira</h1><button className="text-button danger" disabled={busy || loading || Boolean(error)} onClick={() => setConfirmation('all')}><Icon name="trash" />Excluir tudo</button></header>{items.length ? <ul className="trash-list">{items.map(item => <li key={`${item.command}-${item.listId ?? 'root'}-${item.id}`}><span><strong title={item.title ?? item.name}>{item.title ?? item.name ?? 'Item sem título'}</strong><small>Disponível até {item.purgeAfter?.slice(0, 10) ?? 'prazo indisponível'}</small></span><div className="row-actions"><button aria-label={`Restaurar ${item.title ?? item.name}`} title="Restaurar" disabled={busy} onClick={() => void restore(item)}><Icon name="restore" /></button><button className="danger" aria-label={`Excluir definitivamente ${item.title ?? item.name}`} title="Excluir definitivamente" disabled={busy} onClick={() => setConfirmation(item)}><Icon name="trash" /></button></div></li>)}</ul> : loading ? <p role="status">Carregando a lixeira…</p> : !error && <div className="empty"><p>A lixeira está vazia. Os itens removidos aparecerão aqui por 30 dias.</p></div>}<p role="status">{error || message}</p>{[activities, notes, categories].map((source, index) => source.partial && <button key={index} onClick={source.loadMore}>Carregar mais {['atividades', 'notas', 'categorias'][index]}</button>)}
