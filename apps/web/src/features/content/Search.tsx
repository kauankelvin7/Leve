...
export function Search() {
  ...
  return <main><header className="page-heading"><p className="eyebrow">Conteúdo pessoal</p><h1 id="page-title" tabIndex={-1}>Buscar</h1><p>Atividades, notas e compras.</p></header><label className="search-field">Título ou texto<input type="search" autoFocus value={term} onChange={event => setTerm(event.target.value)} /></label>{query ? results.length ? <ul className="search-results">{results.map(result => <li key={result.key}><Link to={result.to}><strong>{result.title}</strong><small>{result.kind}</small></Link></li>)}</ul> : <div className="empty"><p>Nada encontrado.</p></div> : null}<p role="status">{activities.error || notes.error || lists.error || items.error}</p></main>;
}
