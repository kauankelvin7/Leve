...
export function Login({ mode = 'login' }: { mode?: AuthMode }) {
  ...
  return <div className="auth-page"><AuthBackdrop /><aside className="auth-brand"><Link className="brand" to="/entrar">leve<span>.</span></Link><p>Organize o que importa.<br />Respire o resto.</p></aside><main className="entry auth-entry">
    ...
    {!user && mode === 'recovery' ? <p className="auth-description">Informe o e-mail da sua conta. Se houver um acesso por senha associado a ele, enviaremos as instruções de recuperação.</p> : null}
    {!user && mode === 'login' ? <p className="auth-description">Entre para continuar de onde parou.</p> : null}
    ...
    {user ? <>
      ...
      <form onSubmit={submit}>{mode === 'register' ? <><p className="auth-description">Crie seu acesso com e-mail e senha ou use sua conta Google.</p><label>Seu nome<input name="name" autoComplete="name" required maxLength={80} /></label></> : null}<label>E-mail<input name="email" type="email" autoComplete="email" inputMode="email" placeholder="voce@exemplo.com" required disabled={!configured || busy} /></label>{mode !== 'recovery' ? <PasswordField name="password" label="Senha" autoComplete={mode === 'register' ? 'new-password' : 'current-password'} /> : null}{mode === 'register' ? <><PasswordField name="passwordConfirmation" label="Confirmar senha" autoComplete="new-password" /><small className="field-hint">A senha deve ter pelo menos 8 caracteres. Você confirmará seu e-mail antes de usar a agenda.</small></> : null}<button className="primary auth-submit" disabled={!configured || busy}>{busy ? 'Aguarde…' : mode === 'recovery' ? 'Enviar instruções' : mode === 'register' ? 'Criar conta' : 'Entrar'}</button></form>
