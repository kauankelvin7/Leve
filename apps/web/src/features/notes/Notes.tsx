...
import { NoteBody, NoteEditor, readNoteDocument } from './NoteEditor';
...
export function Notes() {
  ...
  return <main><header className="page-heading"><p className="eyebrow">Ideias e registros</p><h1 id="page-title" tabIndex={-1}>Notas</h1><p>Anotações, ideias e lembretes para consultar quando precisar.</p></header>
    ...
    <label className="search-field">Buscar nas notas carregadas<input type="search" value={query} onChange={event => setQuery(event.target.value)} /></label>{loading ? <p role="status">Carregando…</p> : active.length ? <div className="note-grid">{active.map(note => <article className={`note ${note.paperColorPreset}`} key={note.id}><p className="note-kicker">{note.pinned ? 'Fixada no Meu dia' : 'Anotação pessoal'}</p><h2>{note.title}</h2><NoteBody document={note.bodyDoc} />{note.linkedDate ? <small>Vinculada a {note.linkedDate}</small> : null}<div className="dialog-actions"><button disabled={busy} onClick={() => void beginEdit(note)}>Editar</button><button disabled={busy} onClick={() => void trash(note)}>Mover para lixeira</button></div></article>)}</div> : <div className="empty"><p>{normalizedQuery ? 'Nenhuma nota carregada corresponde à busca.' : 'Você ainda não criou notas.'}</p></div>}
