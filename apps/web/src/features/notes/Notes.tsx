...
export function Notes() {
  ...
  const [paperPreset, setPaperPreset] = useState<NoteColorPreset>(emptyPayload.paperColorPreset);
  ...
  useEffect(() => {
    ...
    void readDraft<NoteDraft>(user.uid, 'note:new').then(draft => {
      ...
      entityId.current = draft.entityId; revision.current = draft.expectedRevision; setInitial(draft.payload); setPaperPreset(draft.payload.paperColorPreset); setFormVersion(value => value + 1);
  ...
  function resetComposer() {
    ...
    setInitial(emptyPayload); setPaperPreset(emptyPayload.paperColorPreset); setFormVersion(value => value + 1);
  }

  function startNewNote() {
    resetComposer();
    setMessage('');
    window.setTimeout(() => document.querySelector<HTMLInputElement>('.note-composer input[name=title]')?.focus(), 0);
  }
  ...
  async function trash(note: StoredNote) {
    ...
    try {
      await sendCommand({ command: 'note.trash', operationId: crypto.randomUUID(), entityId: note.id, expectedRevision: note.revision, payload: {} });
      setMessage('Nota movida para a lixeira.');
    } catch (failure) {
      setMessage(failure instanceof Error ? failure.message : 'Não foi possível mover a nota para a lixeira. Tente novamente.');
  ...
  return <main className="notes-page"><header className="page-heading notes-heading"><div><p className="eyebrow">Ideias e registros</p><h1 id="page-title" tabIndex={-1}>Notas</h1><p>Guarde o que quiser lembrar.</p></div><div className="notes-heading-actions"><span className="notes-count" aria-live="polite">{loading ? 'Carregando…' : normalizedQuery ? `${active.length} ${active.length === 1 ? 'resultado' : 'resultados'}` : `${active.length} ${active.length === 1 ? 'nota ativa' : 'notas ativas'}`}</span><button onClick={startNewNote}><Icon name="plus" />Nova nota</button></div></header>
    <section className="panel content-form note-composer" data-note-preset={paperPreset}><h2>{editing ? 'Editar nota' : 'Nova nota'}</h2><form ref={formRef} key={formVersion} onSubmit={submit} onInput={scheduleAutosave} onChange={scheduleAutosave}><label>Título<input name="title" required maxLength={100} placeholder="Ex.: ideias da aula" defaultValue={initial.title} /></label><label>Texto<NoteEditor initial={initial.bodyDoc} editorRef={editorRef} onInput={scheduleAutosave} /></label><details className="optional-fields" open={Boolean(editing || initial.linkedDate || initial.linkedActivityIds.length || initial.pinned)}><summary>Organização <span>opcional</span></summary><div className="optional-fields-content"><div className="date-fields"><label>Estilo da nota<select name="paperColorPreset" defaultValue={initial.paperColorPreset} onChange={event => setPaperPreset(event.target.value as NoteColorPreset)}>{notePresetIds.map(value => <option key={value} value={value}>{noteColorPresets[value].label}</option>)}</select></label><label>Vincular ao dia<input type="date" name="linkedDate" defaultValue={initial.linkedDate ?? ''} /></label></div>{linkableActivities.length ? <fieldset><legend>Atividades vinculadas</legend>{linkableActivities.map(activity => <label className="check-label" key={activity.id}><input type="checkbox" name="linkedActivityIds" value={activity.id} defaultChecked={initial.linkedActivityIds.includes(activity.id)} /> {activity.title}</label>)}</fieldset> : <p className="field-hint">Nenhuma atividade para vincular.</p>}<label className="check-label"><input type="checkbox" name="pinned" defaultChecked={initial.pinned} /> Fixar no Meu dia</label></div></details><div className="dialog-actions"><button className="primary" disabled={busy}>{editing ? 'Concluir edição' : 'Salvar nota'}</button>{editing ? <button type="button" onClick={resetComposer}>Cancelar</button> : null}</div></form>
    ...
    {(error || message) ? <p role={error ? 'alert' : 'status'} className="form-status note-feedback" aria-live="polite">{error || message}</p> : null}
    <label className="search-field notes-search">Buscar nas notas<input type="search" placeholder="Digite uma palavra ou título" value={query} onChange={event => setQuery(event.target.value)} /></label>{loading ? <p role="status">Carregando suas notas…</p> : active.length ? <div className="note-sections">{pinned.length ? <section aria-labelledby="pinned-notes"><h2 id="pinned-notes">Fixadas <span>{pinned.length}</span></h2><div className="note-grid">{pinned.map(noteCard)}</div></section> : null}{others.length ? <section aria-labelledby="other-notes"><h2 id="other-notes">Outras notas <span>{others.length}</span></h2><div className="note-grid">{others.map(noteCard)}</div></section> : null}</div> : <div className="empty notes-empty"><p>{normalizedQuery ? 'Nenhuma nota encontrada.' : 'Nenhuma nota ainda.'}</p>{!normalizedQuery ? <button className="text-link" type="button" onClick={startNewNote}>Criar primeira nota</button> : null}</div>}
</main>;
}
