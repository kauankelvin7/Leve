...
export function Notes() {
  ...
  return <main><header className="page-heading"><p className="eyebrow">Ideias e registros</p><h1 id="page-title" tabIndex={-1}>Notas</h1><p>Anotações, ideias e lembretes para consultar quando precisar.</p><button onClick={() => document.querySelector<HTMLInputElement>('input[name=title]')?.focus()}><Icon name="plus" />Nova nota</button></header>
    ...
    <label className="search-field">Buscar nas notas carregadas<input type="search" value={query} onChange={event => setQuery(event.target.value)} /></label>{loading ? <p role="status">Carregando suas notas…</p> : active.length ? <div className="note-sections">{pinned.length ? <section aria-labelledby="pinned-notes"><h2 id="pinned-notes">Fixadas</h2><div className="note-grid">{pinned.map(noteCard)}</div></section> : null}{others.length ? <section aria-labelledby="other-notes"><h2 id="other-notes">Outras notas</h2><div className="note-grid">{others.map(noteCard)}</div></section> : null}</div> : <div className="empty"><p>{normalizedQuery ? 'Nenhuma nota combina com esta busca. Tente outra palavra.' : 'Nada escrito ainda. Toque em + para começar.'}</p></div>}
