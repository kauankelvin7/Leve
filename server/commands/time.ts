...
import { civilDateSchema, timeEntryManualInputSchema } from '../../packages/domain/src/content.ts';
...
export async function timeEntryCommand(identity: DecodedIdToken, command: CommandEnvelope): Promise<CommandResult> {
  ...
  if (!['start', 'stop', 'addManual', 'trash'].includes(action ?? '')) throw new AppError(422, 'VALIDATION_ERROR', 'Comando desconhecido.');
  const input: Record<string, any> = action === 'start' ? startSchema.parse(command.payload) : action === 'addManual' ? timeEntryManualInputSchema.parse(command.payload) : emptySchema.parse(command.payload);
  ...
  return db.runTransaction(async transaction => {
    const activityRef = action === 'start' || action === 'addManual' ? root.collection('activities').doc(String(input.activityId)) : null;
    ...
    if (action === 'start') {
      ...
      transaction.create(target, { ...input, startedAt, endedAt: now, source: 'manual', revision, schemaVersion: 1, deletedAt: null, createdAt: now, updatedAt: now });
    } else {
